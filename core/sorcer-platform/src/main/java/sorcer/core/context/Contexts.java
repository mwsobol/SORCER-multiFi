...
public class Contexts implements SorcerConstants {
	 ...
	 * </ul>
	 * 
	 * a pcr to this method with contextReturn="a/b" will return both "a/b/c" and
		...
		try {
			...
			for (Object obj : allNodes) {
				if (((ContextNode) obj).getData() instanceof Context) {
					...
					additional = Arrays
							.asList(getTaskContextNodes((Subroutine) obj));
	...
	public static ContextNode[] getTaskContextNodes(Subroutine task)
			throws ContextException {
	...
	public static ContextNode[] getTaskContextNodes(Job job)
		...
		for (Object exertion : exertions) {
			if (exertion instanceof Subroutine) {
				additional = Arrays
						.asList(getTaskContextNodes((Subroutine) exertion));
				if (additional.size() > 0)
					allNodes.addAll(additional);
	...
	public static List getNamedInPaths(Context cntxt) throws ContextException {
		// getValue all the in and inout paths
		String cs = ((ServiceContext)cntxt).getDomainStrategy().getCurrentSelector();
		if (cs != null)
	...
	public static List getNamedOutPaths(Context cntxt) throws ContextException {
		...
		return getPrefixedOutPaths(cntxt, ((ServiceContext)cntxt).getDomainStrategy().getCurrentSelector());
	}
